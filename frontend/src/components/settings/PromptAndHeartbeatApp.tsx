...
const PromptAndHeartbeatApp: React.FC<PromptAndHeartbeatAppProps> = ({ showToast }) => {
  ...
  const runSystemDoctor = async () => {
    ...
    try {
      ...
      if (response.data.summary.health_score >= 80) {
        ...
        showToast('⚠️ System has some warnings');
      } else {
  ...
  return (
    <section className="wrap">
      {/* Section Header with Doctor Button */}
      <div className="section-head reveal" style={{ marginBottom: isMobile ? 16 : 24, paddingTop: 8, paddingBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 12 : 0 }}>
        <div style={{ flex: 1 }}>
          <h2 className="console-title" style={{ marginTop: 4, marginBottom: 8, color: 'var(--accent, #7cf6d3)', fontSize: isMobile ? '1.1rem' : '1.25rem' }}>
            Assistant Settings Configuration
          </h2>
          <p style={{ marginBottom: 4, maxWidth: 600, lineHeight: 1.4, color: 'var(--text)', opacity: 0.9, fontSize: isMobile ? '0.85rem' : '1rem' }}>
      ...
      <section className="panel reveal delay-2 glow-panel" style={{ marginBottom: isMobile ? '16px' : '24px' }}>
      ...
      <div className="grid" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 16 : 24, paddingBottom: "16px" }}>
        {/* Left Column: System Prompt */}
        <SystemPromptPanel showToast={showToast} />
        
        {/* Right Column: Heartbeat */}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ margin: 0, color: 'var(--accent)', fontSize: isMobile ? '1.2rem' : '1.5rem' }}>
            ...
            {isRunningDoctor ? (
              <div style={{ textAlign: 'center', padding: '40px' }}>
                ...
                <p style={{ color: 'var(--text)', fontSize: '1.1rem' }}>Running system diagnostics...</p>
              </div>
            ) : doctorReport ? (
              <>
                {/* Health Score */}
                <div style={{
                  ...
                  <div style={{ fontSize: isMobile ? '2.5rem' : '3rem', fontWeight: 'bold', color: doctorReport.summary.health_score >= 80 ? '#10b981' : doctorReport.summary.health_score >= 60 ? '#f59e0b' : '#ef4444', marginBottom: '8px' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ color: 'var(--accent)', marginBottom: '12px', fontSize: isMobile ? '1rem' : '1.1rem' }}>Diagnostic Checks</h3>
                    ...
                    {doctorReport.checks.map((check, idx) => (
                      <div key={idx} style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
                          <span style={{ fontSize: '1.2rem', flexShrink: 0 }}>{getStatusIcon(check.status)}</span>
                          <div style={{ minWidth: 0, flex: 1 }}>
                            <div style={{ color: 'var(--text)', fontWeight: 600, fontSize: isMobile ? '0.85rem' : '0.95rem' }}>{check.name}</div>
                            <div style={{ color: 'var(--muted)', fontSize: isMobile ? '0.75rem' : '0.85rem', wordBreak: 'break-word' }}>{check.details}</div>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                {doctorReport.errors.length > 0 && (
                  <div style={{ marginBottom: '24px' }}>
                    <h3 style={{ color: '#ef4444', marginBottom: '12px', fontSize: isMobile ? '1rem' : '1.1rem' }}>❌ Critical Issues</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                {doctorReport.warnings.length > 0 && (
                  <div style={{ marginBottom: '24px' }}>
                    <h3 style={{ color: '#f59e0b', marginBottom: '12px', fontSize: isMobile ? '1rem' : '1.1rem' }}>⚠️ Warnings</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>

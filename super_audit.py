...
def scan_static(filepath: Path) -> Dict:
    ...
    try:
        ...
        if filepath.suffix == ".py":
            debug_count = sum(
                1 for l in lines if "print(" in l and not l.strip().startswith("#")
            )
        else:  # .ts, .tsx, .js
            debug_count = sum(
                1
        ...
        funcs = [
            m.group(2) if filepath.suffix != ".py" else m.group(2)
    ...
    for tsx_file in (PROJECT_ROOT / "frontend" / "src").rglob("*.tsx"):
        ...
        try:
            ...
            for call in re.findall(r'apiClient\.delete\([\'"]([^\'"]+)[\'"]', text):
                ...
                if not any(
                    base.startswith(ep.rsplit("/", 1)[0]) for ep in delete_endpoints
                ):
                    issues.append(
                        f"❌ Missing DELETE endpoint: {call} in {tsx_file.name}"
                    )
        except Exception:
            pass
    ...
    for tsx_file in (PROJECT_ROOT / "frontend" / "src").rglob("*.tsx"):
        ...
        try:
            ...
            missing = (
                handlers_called
                - handlers_defined
                - {"handleClick", "handleSubmit", "handleKeyDown"}
            )
            for m in missing:
...
def run_react_audit() -> Dict:
    ...
    COMPONENT_DEF_PATTERNS = [
        ...
        re.compile(
            r"export\s+const\s+([A-Z][A-Za-z0-9_]+)\s*=\s*(?:\$\$[^)]*\$\$|[A-Za-z0-9_]+)\s*=>"
        ),
    ...
    duplicates = {
        name: paths for name, paths in component_to_path.items() if len(paths) > 1
    }
    unused = [
        {"component": name, "path": paths[0]}
        for name, paths in component_to_path.items()
    ...
    try:
        cmd = (
            f'npx prettier --check "{FRONTEND_SRC}/**/*.{{js,jsx,ts,tsx,json,css,md}}"'
        )
        stdout, stderr, code = run_shell_command(cmd, cwd=PROJECT_ROOT)
        if code != 0:
    ...
    try:
        stdout, stderr, code = run_shell_command(
            f"fdupes -r {FRONTEND_SRC}", cwd=PROJECT_ROOT
        )
        if code == 0:
            groups = []
            current = []
...
def build_project_summary(
    static_results: List[Dict], runtime_issues: List[str], react_report: Dict
) -> Dict:
...
def llm_analyze(summary: Dict, model: str) -> str:
    ...
    llm = ChatOllama(model=model, temperature=0.1, num_predict=4096, timeout=600)
...
def main():
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Trigger self‑healing after audit"
    )
    parser.add_argument(
        "--output", type=str, default=str(AUDIT_REPORT_MD), help="Output markdown file"
    )
    ...
    md_lines.append("\n## React Frontend Audit")
    md_lines.append(
        f"- Duplicate components: {len(react.get('duplicate_components', {}))}"
    )
    md_lines.append(f"- Unused components: {len(react.get('unused_components', []))}")
    md_lines.append(
        f"- ESLint errors: {react.get('eslint', {}).get('total_errors', 0)}"
    )
    md_lines.append(
        f"- Prettier files needing formatting: {react.get('prettier', {}).get('files_needing_formatting', 0)}"
    )

...
<p align="center">
  <strong>Current Version 1.7.0 | Effective Initial Release Date: June 2026</strong><br />
...
#### Roles System — Major Feature
- **Role-Based Agent Configuration**: Each Role is a self-contained agent with its own system prompt, LTM collection, citation setting, and optional model override.
- **Role CRUD UI**: Full create/edit/delete interface accessible via the **⚙ Roles** button in the Chat Console toolbar.
- **Role Selector Dropdown**: Quick-switch the active role from the toolbar. The selector auto-restores when you switch threads.
- **RolesModal**: A dedicated modal for creating, editing, and deleting roles. Includes collection dropdown, model override picker, citation toggle, and system prompt editor.
- **Roles REST API**: New endpoints under `/api/v1/roles` — list, get, create, update, delete, activate, get-active. Registered via `app/api/v1/router.py`.
- **Default Role**: A "Default" role is seeded on first launch; it cannot be deleted.

#### Role-Scoped Memory
- New `user_facts.role_id` column. Facts learned while a role is active are tagged with that role and only visible to it.
- Global facts (`role_id IS NULL`) remain visible to every role.
- `get_user_profile_context(role_id=...)` filters facts: a role sees global facts + its own facts only.

#### Thread ↔ Role Binding
- New `chat_threads.role_id` column. Each chat thread remembers the role that created it.
- Switching threads auto-restores the correct role in the dropdown.
- The thread's role is preserved on every message via `COALESCE` — it can be set once and never accidentally overwritten.

#### ⚙️ Assistant Settings Integration
- `SystemPromptPanel` now detects the active role and defaults to editing **that role's** prompt.
- A mode toggle (` Role: X` / `🌐 Global`) lets you switch between editing the role prompt and the global fallback.
- Save button routes to the correct destination based on the current mode.

#### Citation Control & Anti-Hallucination
- **Citations Toggle**: New Red/Green button in the Chat Console to enable or disable footnotes per request.
- **Conditional Citation Rules**: Citation rules are now injected based on `citation_mode`:
  - `rag` — knowledge base context present → full inline citations + References section.
  - `file` — attached file present → inline `(p. X)` page refs only, no References section.
  - `none` — plain chat → no citation markers at all.
- **Removed hallucinated `[Source N]` markers** that were appearing in plain chat (the previous anti-citation clause named the exact format, which paradoxically made models emit it).
- `_build_stm_context_for_prompt()` no longer contains any citation instructions.

#### Model Warmup & Loading UX
- New `POST /api/models/warmup` endpoint forces Ollama to load a model into RAM ahead of the first request.
- Frontend triggers warmup automatically when you change the model selector.
- The "LLM is thinking..." indicator now escalates with helpful messages at 5s, 30s, and 2min — telling the user that large CPU-bound models are loading.

#### Backend Fixes
- **Auto-save bug fixed**: The `INSERT INTO chat_messages` for user messages was missing a `?` placeholder, causing every chat to fail silently and disappear on refresh. All chats are now persisted immediately; only the thread-pane `×` deletes them.
- **`/api/chat` endpoint cleaned**: removed debug `print()` statements, dead `_build_stm_context()` and `get_system_prompt()` calls, and redundant LLM instantiation.
- **Migration chain fixed**: Duplicate `add_roles_table` migration removed; `b2c3d4e5f6a7` re-parented to the baseline. Fresh installs now run migrations cleanly.
- **Default model save**: Direct SQLite write with a 10s timeout to survive uvicorn hot-reloads.
- **Chunk cache preserved**: Ingestion chunks are no longer auto-deleted after processing; users clean up manually at `~/MAi-RAG-PA/storage/chunk_cache/`.

#### Ingestion Fixes (carried into this release)
- Case-insensitive extension matching (`.PDF`, `.Pdf`, `.pdf` all accepted).
- Binary formats (PDF, EPUB, DOCX, PPTX, XLSX) no longer rejected for containing null bytes.
- Magic-byte validation for common binary formats.

#### Embedding Model — Major Important Feature Upgrade
- Embeddings now use Ollama's `nomic-embed-text` (768-dim) instead of the local `all-MiniLM-L6-v2` sentence transformer.
- Fresh MAi-RAG-PA installation installs nomic-embed-text embedding agent by default
- The `models/all-MiniLM-L6-v2/` folder is no longer required.
- **Migration required for existing installs**: delete old Qdrant collections and re-ingest (384 → 768 dimensions).

#### Housekeeping
- Removed stale build artifacts from `frontend/public/assets/`.
- Removed runtime files (`report.md`, `super_audit_report.json`, `.coverage`, `qdrant.log`) from sync.
- `sync-to-release.sh` now excludes `.aider*`, `file_chat.py`, `00_convert_mobi.py`, `MAi-RAG-PA-v*.tar.gz`, and `*.dmp`.

#### Files Added in this Release
- `alembic/versions/b2c3d4e5f6a7_add_roles_table.py`
- `alembic/versions/c3d4e5f6a7b8_add_role_id_to_user_facts.py`
- `app/api/v1/roles.py`
- `app/api/v1/router.py`
- `frontend/src/components/chat/RolesModal.tsx`

#### Files Modified in this Release
- `app/agents/agent_core.py` (roles, scoped memory, conditional citations, warmup support)
- `app/main.py` (roles router, auto-save fix, warmup endpoint, cleaner chat endpoint)
- `app/api/v1/router.py` (registers roles router)
- `app/memory/qdrant_manager.py` (Ollama embeddings)
- `app/rag/rag_core.py` (Ollama embeddings)
- `app/config.py` (embedding model constants)
- `frontend/src/components/chat/ChatConsoleApp.tsx` (role dropdown, warmup, loading hint, citations toggle, thread-role binding)
- `frontend/src/components/settings/SystemPromptPanel.tsx` (role-aware editing)
- `index.html` (SEO updated for new features)
- `install.sh` (pull `nomic-embed-text:latest`)
- `sync-to-release.sh` (updated exclusions)

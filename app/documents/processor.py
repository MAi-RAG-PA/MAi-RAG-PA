# ~/MAi-RAG/app/documents/processor.py
import os
import re
import json
import csv
...
def process_pdf(pdf_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages, start=1):
                try:
                    ...
                    logger.warning(f"Skipping page {page_num} in {pdf_path}: {e}")
        ...
        logger.info(f"PDF '{pdf_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
...
def process_epub(epub_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        for item in book.get_items():
            if item.get_type() == ITEM_DOCUMENT:
                try:
                    ...
                    logger.warning(f"Skipping item in EPUB {epub_path}: {e}")
        ...
        logger.info(f"EPUB '{epub_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
...
def process_txt(txt_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        logger.info(f"TXT '{txt_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process TXT {txt_path}: {e}")
...
def process_docx(docx_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        logger.info(f"DOCX '{docx_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
...
def process_html(html_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        full_text = normalize_text(full_text)
        if not full_text:
            logger.warning(f"No text extracted from HTML {html_path}")
            return chunks
        ...
        logger.info(f"HTML '{html_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
...
def process_markdown(md_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        full_text = normalize_text(full_text)
        if not full_text:
            logger.warning(f"No text extracted from Markdown {md_path}")
            return chunks
        ...
        logger.info(f"Markdown '{md_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process Markdown {md_path}: {e}")
...
def process_rtf(rtf_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if not full_text:
            logger.warning(f"No text extracted from RTF {rtf_path}")
            return chunks
        ...
        logger.info(f"RTF '{rtf_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process RTF {rtf_path}: {e}")
...
def process_odt(odt_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if not full_text:
            logger.warning(f"No text extracted from ODT {odt_path}")
            return chunks
        ...
        logger.info(f"ODT '{odt_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process ODT {odt_path}: {e}")
...
def process_csv(csv_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        logger.info(f"CSV '{csv_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
...
def process_json(json_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if not full_text:
            logger.warning(f"No text extracted from JSON {json_path}")
            return chunks
        ...
        logger.info(f"JSON '{json_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
...
def process_xml(xml_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if not full_text:
            logger.warning(f"No text extracted from XML {xml_path}")
            return chunks
        ...
        logger.info(f"XML '{xml_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process XML {xml_path}: {e}")
...
def process_pptx(pptx_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if not full_text:
            logger.warning(f"No text extracted from PPTX {pptx_path}")
            return chunks
        ...
        logger.info(f"PPTX '{pptx_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process PPTX {pptx_path}: {e}")
...
def process_xlsx(xlsx_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if not full_text:
            logger.warning(f"No text extracted from XLSX {xlsx_path}")
            return chunks
        ...
        logger.info(f"XLSX '{xlsx_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process XLSX {xlsx_path}: {e}")
...
def process_tex(tex_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if LATEX_AVAILABLE:
            ...
            # Basic LaTeX stripping without pylatexenc
        ...
        if not full_text:
            logger.warning(f"No text extracted from LaTeX {tex_path}")
            return chunks
        ...
        logger.info(f"LaTeX '{tex_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process LaTeX {tex_path}: {e}")
...
def process_rst(rst_path: Path, max_words: int = 300) -> List[Dict]:
    ...
    try:
        ...
        if RST_AVAILABLE:
            ...
            # Basic RST stripping without docutils
        ...
        if not full_text:
            logger.warning(f"No text extracted from RST {rst_path}")
            return chunks
        ...
        logger.info(f"RST '{rst_path.name}' split into {len(chunks)} chunks")
    except Exception as e:
        logger.error(f"Failed to process RST {rst_path}: {e}")
...
def process_directory(directory: Path, max_words: int = 300, force_reprocess: bool = False) -> List[Dict]:
    ...
    logger.info(f"Processed {processed_count} files ({skipped_count} unchanged) → {len(all_chunks)} chunks from {directory}")
    return all_chunks

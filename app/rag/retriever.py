...
class Retriever:
    ...
    def __init__(self, collection_name: str = "local_docs"):
        ...
        try:
            ...
            logger.warning("QdrantMemoryManager not available: %s", e)
            logger.warning("RAG features will be disabled")
        except Exception as e:
            logger.warning("Qdrant not available for Retriever: %s", e)
...
class AdvancedRetriever:
    ...
    def __init__(self):
        ...
        try:
            ...
            if self.qdrant.client:
                ...
                self.qdrant_available = True
                logger.info(
                    "Qdrant connection successful - %s collections available",
                    len(self.available_collections),
                )
                if self.available_collections:
                    logger.info(
                        "Collections: %s", ", ".join(self.available_collections)
                    )
            else:
                raise RuntimeError("Qdrant client unavailable")
        except ImportError as e:
            ...
            logger.warning("Qdrant not available for AdvancedRetriever: %s", e)
            logger.warning("RAG features will be disabled until Qdrant is running")
            logger.warning(
                "To enable RAG: Start Qdrant and ensure it's accessible at localhost:6333"
            )
        ...
        for word in words:
            if word in synonyms:
                for syn in synonyms[word]:
                    candidate = query.replace(word, syn)
    ...
    def retrieve_advanced(
        ...
        for q in queries:
            try:
                ...
                logger.warning("Query expansion search failed for '%s': %s", q, e)
                continue
        ...
        for result in sorted_results:
            ...
            dedupe_key = result_id if result_id is not None else result.get("text", "")
        ...
        logger.info(
            "RAG retrieval: %s unique results from %s", len(unique_results), collection
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_collection": self.get_best_collection()

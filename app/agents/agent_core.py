...
def get_protected_models_status() -> List[Dict[str, Any]]:
    ...
    for model in PROTECTED_SYSTEM_MODELS:
        is_installed = model["name"].lower() in installed_models
        model_status.append(
            {
                **model,
...
WORKSPACE: Path = Path.home() / "MAi-RAG-PA" / "workspace"
PROJECT_ROOT: Path = Path.home() / "MAi-RAG-PA"
...
def initialize_dev_sandbox() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "message": f"Sandbox created with {len(copied_items)} items",
        }
...
def _get_llm(
    ...
    cache_key = f"{model_name}_{temperature}_{repeat_penalty}_{num_predict}_{timeout}"
